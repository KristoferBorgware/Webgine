// Right-handed perspective camera. Produces view/projection matrices; owns no GPU state.
// Aspect ratio is supplied per-frame by the renderer. Column-vector / WebGPU-native.
// Extends Node, so a camera lives in the scene graph and picks up a world transform from
// its ancestors.
//
// view() is overridable so derived cameras (e.g. FreeFloatCamera, OrbitCamera) supply
// their own orientation while sharing the projection. The `active` flag marks which
// camera the scene renders through.
...
export class Camera extends Node {
  ...
  screenPointToRay(px: number, py: number, viewportW: number, viewportH: number): Ray {
    // Pixel -> normalized device coords (x, y in [-1, 1], y flipped, z in [0, 1]).
